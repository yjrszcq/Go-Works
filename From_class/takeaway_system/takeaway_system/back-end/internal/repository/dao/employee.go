...
type Employee struct {
	...
	Status     string    `gorm:"type:enum('可用', '不可用');default:'不可用';comment:员工状态"`
...
func (dao *EmployeeDAO) FindEmployeeByRole(ctx context.Context, role string) ([]Employee, error) {
	var employees []Employee
	err := dao.db.WithContext(ctx).Where("role = ?", role).Find(&employees).Error
	return employees, err
}

func (dao *EmployeeDAO) FindEmployeeByStatus(ctx context.Context, status string) ([]Employee, error) {
	var employees []Employee
	err := dao.db.WithContext(ctx).Where("status = ?", status).Find(&employees).Error
	return employees, err
}
